...
<head>
    ...
    <script type="text/javascript"
            src="<c:url value="http://ajax.googleapis.com/ajax/libs/jqueryui/1.11.1/jquery-ui.min.js"/>"></script>
    <script type="text/javascript"
            ...
            <div class="col s12">
                ...
                <c:choose>
                        ...
                        <div class="row">
                            <div class="players">
                                <div class="col s12">
                                    ...
                                    <form:form id="addPlayer" modelAttribute="playerForm" action="${postPath}"
                                               method="post">
                                        <h5><spring:message code="tournament.player.add.title"/></h5>
                                        <div class="row" style=" margin-left: 10px;">
                                            <div class="input-field col s3">
                                                <form:label path="player"><spring:message code="tournament.player"/>:
                                                </form:label>
                                                <spring:message code="tournament.player.placeholder"
                                                                var="playerPlaceholder"/>
                                                <form:input placeholder="${playerPlaceholder}" type="text" path="player"
                                                            autocomplete="off"/>
                                                <form:errors path="player" cssClass="form-error" element="p"/>
                                            </div>
                                            <div class="input-field col s3">
                                                <form:label path="username"><spring:message
                                                        code="tournament.player.username"/>: </form:label>
                                                <spring:message code="tournament.player.username.placeholder"
                                                                var="usernamePlaceholder"/>
                                                <form:input placeholder="${usernamePlaceholder}" type="text"
                                                            path="username" autocomplete="off"/>
                                                <form:errors path="username" cssClass="form-error" element="p"/>
                                            </div>
                                            <div class="col s2">
                                                <button id="addButton" class="btn btn-primary light-blue darken-4"
                                                        style="margin-top: 20px" type="submit"><spring:message
                                                        code="tournament.player.add"/></button>
                                            </div>
                                        </div>
                                    </form:form>
                                    <div class="row center">
                                        <c:if test="${tournament.status == 'NEW' && tournament.players.size() >= 2}">
                                            <form action="<c:url value="/update/tournament/${tournament.id}/generate"/>"
                                                  method="post">
                                                <button type="submit" class="btn btn-primary light-blue darken-4">
                                                    <spring:message code="tournament.players.generate"/></button>
                                            </form>
                                        </c:if>
                                    </div>
                                    <div class="row">
                                                            ...
                                                            <span class="player-seed">
                                                                ...
                                                                <c:if test="${player.user.name != null}">
                                                                    <span class="user-name">- <c:out
                                                                            value="${player.user.name}"/></span>
                                                                </c:if>
                                                                <form style="width: 0;height: 0; margin: 0; padding: 0; display: inline"
                                                                      id="<c:out value="${seed}"/>"
                                                                      action="<c:url value="/remove/player/${tournament.id}/${player.id}"/>"
                                                                      method="POST">
                                                                    <a href="#" onclick="$('#<c:out
                                                                            value="${seed}"/>').submit()">
                                                        ...
                                                        <c:forEach var="inscription" items="${inscriptions}">
                                                            <li>
                                                                <div class="row">
                                                                    ...
                                                                    <div class="col s1">
                                                                        <form action="<c:url value="/accept/user/${inscription.user.id}/tournament/${tournament.id}"/>"
                                                                              method="post">
                                                                            <button type="submit"
                                                                                    class="btn btn-primary green"><i
                                                                                    class="material-icons">check</i>
                                                                            </button>
                                                                        </form>
                                                                    </div>
                                                                    <div class="col s1">
                                                                        <form action="<c:url value="/reject/user/${inscription.user.id}/tournament/${tournament.id}"/>"
                                                                              method="post">
                                                                            <button type="submit"
                                                                                    class="btn btn-primary red"><i
                    ...
                    <c:when test="${empty tournament.players && tournament.creator.id != loggedUser.id}">
                        <h5 class="center" style="margin-top: 30px;margin-bottom: 100px;"><spring:message
                                code="tournament.info.noPlayers"/></h5>
                    </c:when>
                                            ...
                                            <c:forEach var="player" items="${tournament.players}">
                                                <c:if test="${player.id != -1}">
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a href="<c:url value="/user/${player.user.id}"/>"><c:out
                                                                    value="${player.user.name}"/></a>
                                                        </td>
                                                    </tr>
                                                    <c:set var="seed" value="${seed+1}"/>
                                                </c:if>
