...
public class TournamentJdbcDao implements TournamentDao {
    ...
    private void removePlayersFromTournament(long tournamentId, List<Player> players) {
        final Tournament tournament = findById(tournamentId);
        long playerId;
        tournament.removePlayer(players);
        for (Player player: players) {
            if(tournament.getPlayers().contains(player)) {
                playerId = player.getId();
                jdbcTemplate.update("DELETE FROM participates_in WHERE player_id=? AND tournament_id=?", playerId, tournamentId);
            }
        }
    }
